...
class MinimaxPlayer(Game, Player):
    ...
    def getMove(self, board):
        ...
        move = self.MiniMax(1, self.max_depth, self.side, board)
        return move

    def MiniMax(self, depth, maxDepth, cur_side, board):
        ...
        if my_turn:
            ...
            bestValue = float("-inf")
            for i in range(0, n):
                val = self.MiniMax(depth + 1, maxDepth, self.opponent(cur_side), self.nextBoard(board, cur_side, moves[i]))
                if val > bestValue:
                    bestValue = val
                    bestMove = copy.deepcopy(moves[i])
    ...
    def evaluation(self, board):
        w_plays = self.side == 'W'
        if w_plays:
            side_moves = self.generateMoves(board, 'W')
            opponent_moves = self.generateMoves(board, 'B')
        else:
            side_moves = self.generateMoves(board, 'B')
            opponent_moves = self.generateMoves(board, 'W')
        return len(side_moves) - len(opponent_moves)
...
class AlphaBetaPlayer(Game, Player):
    ...
    def getMove(self, board):
        ...
        move = self.MiniMax(1, self.max_depth, self.side, board, float("-inf"), float("inf"))
        return move

    def MiniMax(self, depth, maxDepth, cur_side, board, alpha, beta):
        my_turn = cur_side == self.side
        if depth == maxDepth:
            return self.evaluation(board)
        ...
        if my_turn:
            ...
            bestValue = float("-inf")
            for i in range(0, n):
                val = self.MiniMax(depth + 1, maxDepth, self.opponent(cur_side), self.nextBoard(board, cur_side, moves[i]), alpha, beta)
                if val > bestValue:
                    bestValue = val
                    bestMove = copy.deepcopy(moves[i])
...
if __name__ == '__main__':
    ...
    game = Game(8)
    human1 = MinimaxPlayer(8, 6)
    human1.initialize('B')
    human2 = MinimaxPlayer(8, 6)
    human2.initialize('W')
    game.playOneGame(human1, human2, True)
    # game.playNGames(20, human1, human2, True)
